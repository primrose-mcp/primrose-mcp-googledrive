 ...
 * - X-Google-Base-URL: (Optional) Override the default Google Drive API base URL
 */
...
export interface TenantCredentials {
  /** OAuth Access Token for Google Drive API (from X-Google-Access-Token header) */
  accessToken: string;

  /** Override Google Drive API base URL (from X-Google-Base-URL header) */
  baseUrl?: string;
}
...
export function parseTenantCredentials(request: Request): TenantCredentials {
  ...
  return {
    accessToken: headers.get('X-Google-Access-Token') || '',
    baseUrl: headers.get('X-Google-Base-URL') || undefined,
  };
}
...
export function validateCredentials(credentials: TenantCredentials): void {
  if (!credentials.accessToken) {
    throw new Error('Missing credentials. Provide X-Google-Access-Token header.');
  }
}
